...
This report processes the “Predict Students’ Dropout and Academic Success” dataset, developed by Martins et al. (2021). It includes feature engineering and utilizes advanced machine learning models to predict student outcomes: college graduation, dropout, or enrollment. 

College is not only an expensive endeavor but also predictive of long-term health outcomes and income. As such, being able to predict dropout before it happens could enable early intervention programs that support students toward success. With universities often having thousands of students and only a few counselors, a predictive model like the one developed here could be immensely helpful in giving counselors a prioritized starting point for their efforts.
...
packages <- c("readr", "gbm", "rpart.plot", "tinytex", "dplyr", "caret", "rpart", "stringr", "tidyverse", "tidyr", "broom", "glmnet", "Matrix","coefplot", "here")
...
```{r Unzip and Load Data from Github Repo Zip Filem, echo=TRUE}
# #Unzip file
############################################??
...
As seen in the table below, only Target is not encoded numerically. 
...
Now before any analysis or exploration can be done, the best practice is to split the dataset in order to avoid over training.
...
The coefficient plot above provides insights into the variables' relationships with the target outcome and their relative strengths. It highlights variables with significant predictive power, as well as those with wide error margins crossing zero, suggesting limited or no effect.
...
The chart above shows that the vast majority of students are not debtor (don't owe money to school), nor special needs, most are male, aren't international, or scholarship holders. This finding warrants more digging, since the regression above found all but gender to be predictive of our target variable.
...
```
As seen in the decision tree, the model used two key variables to make it's classificaitons: - curricular_units_2nd_sem_approved
...
plot(gbt_model)
```
The plot shows that boosting iteraction (x-axis) did greatly impact accuracy (as seen on y axis) for all three outcomes (dropout=1, enrolled =2, graduated =3). Cross validation automatically picked the best tune that optimizes for the best outcoems across all 2. The chart shows that the three values align best around 200 trees, a depth of 3, shrinkage of .05 and 10 obsevations per node.
...
With the final holdout set now having the same treatment as the traindata set, predictions can be made.
...
# redict with Gradient Boosted Trees Model
